...
nod_threshold = 15  # Threshold for detecting nodding
nod_cooldown = 1.0  # Cooldown period for nod detection in seconds
last_nod_time = 0  # To keep track of the last time a nod was detected
...
# Moving average to smooth out chin position
chin_position_history = []
...
while True:
    ...
    for rect in rects:
        ...
        if prev_chin_center is not None:
            chin_movement = chin_center[1] - prev_chin_center[1]  # Vertical movement only

            # Use a moving average to smooth out chin position fluctuations
            chin_position_history.append(chin_center[1])
            if len(chin_position_history) > 5:  # Adjust the window size as needed
                chin_position_history.pop(0)
            smoothed_chin_position = np.mean(chin_position_history)

            # Log downward nods only
            if chin_center[1] < smoothed_chin_position - nod_threshold:
                current_time = time.time()
                if current_time - last_nod_time > nod_cooldown:  # Cooldown check
                    print("User Nodded Down")
                    last_nod_time = current_time  # Update last nod time

            # Log head movement
            if abs(chin_movement) > movement_threshold:
        ...
        face_width = rects[0].width()  # Width of the detected face
